...
fn main() {
    ...
    for i in 0..n {
        ...
        towns.push((a[0], a[1], i));
    }

    let mut edges = vec![];
    towns.sort_by(|a, b| a.0.partial_cmp(&b.0).unwrap());

    for i in 1..n {
        edges.push(Edge {
            from: towns[i - 1].2,
            to: towns[i].2,
            dist: (towns[i - 1].0 - towns[i].0).abs(),
        });
    }
    towns.sort_by(|a, b| a.1.partial_cmp(&b.1).unwrap());

    for i in 1..n {
        edges.push(Edge {
            from: towns[i - 1].2,
            to: towns[i].2,
            dist: (towns[i - 1].1 - towns[i].1).abs(),
        });
    }

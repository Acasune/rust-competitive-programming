...
fn main() {
    ...
    sc.new_line();
    let n = sc.get::<usize>();
    let mut graph = vec![Vec::<(i32, i32)>::new(); n];
    for _ in 0..n {
        sc.new_line();
        let a = sc.get::<usize>();
        let k = sc.get::<usize>();
    ...
    while !heap.is_empty() {
        ...
        for i in 0..graph[v as usize].len() {
            let (c, v2) = graph[v as usize][i];

            if dist[v2 as usize] > c + dist[v as usize] {
                dist[v2 as usize] = c + dist[v as usize];
                heap.push((-c, v2 as i32));
            }
        }
    }

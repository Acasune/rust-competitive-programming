...
impl Move {
    ...
    fn iter_from_pos(pos: u32) -> impl Iterator<Item = Self> {
        let u = if pos >= 3 { Some(Self::Up) } else { None };
        let d = if pos < 6 { Some(Self::Down) } else { None };
        let l = if pos % 3 >= 1 { Some(Self::Left) } else { None };
        let r = if pos % 3 < 2 { Some(Self::Right) } else { None };
